...
class LSv3_class(object):
    ...
    def LSv3_core_apply(self,core_input, core_all_queue):
            ...
            for core_b in core_a:
                ...
                if self.switch:
                    ...
                    while not core_all_queue[core_area_cen[0]][core_area_cen[1]].empty():
                        ...
                        if self.V[trs_data[0],trs_data[1],trs_data[2]] == self.matrix_value:
                            self.V[trs_data[0],trs_data[1],trs_data[2]] = trs_data[3]
                                ...
                                if self.switch:
                                    ...
                                    if len(target_queue) > 0:
                                        for tqi in target_queue:
                                            core_all_queue[core_area_nei[tqi][0]][core_area_nei[tqi][1]].put([0,local_x,local_y, self.V[0,local_x,local_y]])
                            ...
                            for jj in range(-self.halfL+kk*2,self.halfL+1-kk*2):
                                ...
                                if self.V[kk,local_x,local_y] == self.matrix_value:
                                    ...
                                    if self.switch:
                                        ...
                                        if len(target_queue) > 0:
                                            for tqi in target_queue:
                                                core_all_queue[core_area_nei[tqi][0]][core_area_nei[tqi][1]].put([kk,local_x,local_y, self.V[kk,local_x,local_y]])
    ...
    def LSv3_main(self):
        ...
        for ki in range(main_wc):
            for kj in range(main_lc):
                res_one = pool.apply_async(func = self.LSv3_core_apply, args = (multi_input[ki][kj], all_queue, ), callback=self.res_back )
...
if __name__ == '__main__':
    ...
    for cores in [1]:
        ...
        for nsteps in range(1,11): 
            test1 = LSv3_class(nx,ny,ng,cores,nsteps,P0,R)
            test1.LSv3_main()
            P = test1.get_P()
            V = test1.V
