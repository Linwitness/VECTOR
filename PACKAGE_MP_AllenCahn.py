...
class ACv3_class(object):
    ...
    def ACv3_core_apply(self,core_input, core_all_queue):
    ...
    def ACv3_main(self):
        ...
        for ki in range(main_wc):
            for kj in range(main_lc):
                res_one = pool.apply_async(func = self.ACv3_core_apply, args = (multi_input[ki][kj], all_queue, ), callback=self.res_back )
...
if __name__ == '__main__':
    ...
    for cores in [1]:
        for nsteps in range(2,21,2):
            ...
            test1 = ACv3_class(nx,ny,ng,cores,nsteps,P0,R)
            test1.ACv3_main()
            P = test1.get_P()
