...
# -*- coding: utf-8 -*-
"""
Test suite for 3D Linear smoothing algorithm implementation.
...
from . import (CONFIG_3D, ALGORITHM_PARAMS, PLOT_CONFIG,
               calculate_normal_vector_error_3d, calculate_curvature_error_3d)
...
def test_sphere():
    ...
    alg_params = ALGORITHM_PARAMS['3dlinear']
    ...
    linear3d = linear3d_class(nx, ny, nz, ng, cores, nsteps, P0, R, 
    ...
    linear3d.linear3d_main(purpose="inclination")
    P = linear3d.get_P()
    gb_sites = linear3d.get_gb_list()
    ...
    print(f"Normal vector calculation time: {linear3d.running_time:.2f}s")
    ...
    linear3d = linear3d_class(nx, ny, nz, ng, cores, nsteps, P0, R,
                            ...
                            verification_system=alg_params['verification_system'],
                            curvature_sign=alg_params['curvature_sign'])
    linear3d.linear3d_main(purpose="curvature")
    C = linear3d.get_C()
    
    # Calculate curvature errors
    rms_error, max_error, avg_k, std_k = calculate_curvature_error_3d(C, radius, gb_sites)
    ...
    print(f"Curvature calculation time: {linear3d.running_time:.2f}s")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'linear3d_sphere_test.png'))
    plt.close()
...
def test_convergence():
    ...
    for nsteps in step_range:
        ...
        linear3d.linear3d_main(purpose="curvature")
        C = linear3d.get_C()
        rms_error, _, _, _ = calculate_curvature_error_3d(C, radius, gb_sites)
        curvature_errors.append(rms_error)
        
        times.append(linear3d.running_time)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'linear3d_convergence_test.png'))
    plt.close()

if __name__ == '__main__':
    print("Running 3D Linear smoothing algorithm tests...")
