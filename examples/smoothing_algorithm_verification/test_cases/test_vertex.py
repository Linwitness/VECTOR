...
# -*- coding: utf-8 -*-
"""
Test suite for Vertex smoothing algorithm implementation.
...
import myInput
from PACKAGE_MP_Vertex import vertex_class
...
def test_circle():
    ...
    alg_params = ALGORITHM_PARAMS['vertex']
    nx, ny = cfg['grid_size']
    ng = cfg['num_grains']
    cores = cfg['num_cores']
    ...
    vertex = vertex_class(nx, ny, ng, cores, nsteps, P0, R, 
                        clip=alg_params['clip'],
                        verification_system=alg_params['verification_system'],
                        curvature_sign=alg_params['curvature_sign'])
    vertex.vertex_main(purpose="inclination")
    P = vertex.get_P()
    gb_sites = vertex.get_gb_list()
    ...
    print(f"Normal vector calculation time: {vertex.running_time:.2f}s")
    ...
    vertex = vertex_class(nx, ny, ng, cores, nsteps, P0, R,
                        ...
                        verification_system=alg_params['verification_system'],
                        curvature_sign=alg_params['curvature_sign'])
    vertex.vertex_main(purpose="curvature")
    C = vertex.get_C()
    ...
    print(f"Curvature calculation time: {vertex.running_time:.2f}s")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'vertex_circle_test.png'))
    plt.close()
...
def test_voronoi():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'vertex_voronoi_test.png'))
    plt.close()
...
def test_convergence():
    ...
    for nsteps in step_range:
        ...
        times.append(vertex.running_time)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'vertex_convergence_test.png'))
    plt.close()

if __name__ == '__main__':
    print("Running Vertex smoothing algorithm tests...")
