...
# -*- coding: utf-8 -*-
"""
Test suite for Level Set smoothing algorithm implementation.
...
def test_circle():
    ...
    alg_params = ALGORITHM_PARAMS['levelset']
    nx, ny = cfg['grid_size']
    ng = cfg['num_grains']
    cores = cfg['num_cores']
    ...
    levelset = levelSet_class(nx, ny, ng, cores, nsteps, P0, R,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'levelset_circle_test.png'))
    plt.close()
...
def test_voronoi():
    ...
    plt.subplot(133)
    plt.imshow(C[1,:,:], cmap=PLOT_CONFIG['curvature_colormap'],vmin=0, vmax=1)
    plt.colorbar()
    plt.title("Curvature")
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'levelset_voronoi_test.png'))
    plt.close()
...
def test_convergence():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'levelset_convergence_test.png'))
    plt.close()

if __name__ == '__main__':
    print("Running Level Set smoothing algorithm tests...")
