...
# -*- coding: utf-8 -*-
"""
Test suite for Linear smoothing algorithm implementation.
Tests 2D cases for normal vector and curvature calculations.
...
import myInput
from PACKAGE_MP_Linear import linear_class
...
def test_circle():
    ...
    alg_params = ALGORITHM_PARAMS['linear']
    nx, ny = cfg['grid_size']
    ng = cfg['num_grains']
    cores = cfg['num_cores']
    ...
    linear = linear_class(nx, ny, ng, cores, nsteps, P0, R, 
                         clip=alg_params['clip'],
                         verification_system=alg_params['verification_system'],
                         curvature_sign=alg_params['curvature_sign'])
    linear.linear_main(purpose="inclination")
    P = linear.get_P()
    gb_sites = linear.get_gb_list()
    ...
    print(f"Normal vector calculation time: {linear.running_time:.2f}s")
    ...
    linear = linear_class(nx, ny, ng, cores, nsteps, P0, R,
                         ...
                         verification_system=alg_params['verification_system'],
                         curvature_sign=alg_params['curvature_sign'])
    linear.linear_main(purpose="curvature")
    C = linear.get_C()
    ...
    print(f"Curvature calculation time: {linear.running_time:.2f}s")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'linear_circle_test.png'))
    plt.close()
...
def test_voronoi():
    ...
    for i,j in gb_sites:
        dx, dy = myInput.get_grad(P, i, j)
        plt.arrow(j, i, 10*dx, 10*dy, color='red', width=0.1, alpha=0.5)
    
    plt.subplot(133)
    plt.imshow(C[1,:,:], cmap='coolwarm')
    plt.colorbar()
    plt.title("Curvature")
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'linear_voronoi_test.png'))
    plt.close()
...
def test_convergence():
    ...
    for nsteps in step_range:
        ...
        times.append(linear.running_time)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'linear_convergence_test.png'))
    plt.close()

if __name__ == '__main__':
    print("Running Linear smoothing algorithm tests...")
