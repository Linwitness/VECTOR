        ...
        for mpi in range(main_wc):
            for mpj in range(main_lc):
                for mpk in range(main_hc):
                    # print(f"{mpi} {mpj} {mpk}")
                    res_one = pool.apply_async(func = self.allenCahn3d_normal_vector_core, args = (multi_input[mpi][mpj][mpk], all_queue, ), callback=self.res_back )
...
if __name__ == '__main__':
    ...
    for cores in [8]:
        for nsteps in range(2,21,2):
            ...
            test1 = allenCahn3d_class(nx,ny,nz,ng,cores,nsteps,P0,R)
            test1.allenCahn3d_main()
            P = test1.get_P()
            V = test1.V
