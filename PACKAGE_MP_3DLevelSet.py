        ...
        for mpi in range(main_wc):
            for mpj in range(main_lc):
                for mpk in range(main_hc):
                    res_one = pool.apply_async(func = self.levelSet3d_normal_vector_core, args = (multi_input[mpi][mpj][mpk], all_queue, ), callback=self.res_back )
...
if __name__ == '__main__':
    ...
    for cores in [8]:
        ...
        for nsteps in range(1,11):
            test1 = levelSet3d_class(nx,ny,nz,ng,cores,nsteps,P0,R,'np')
            test1.levelSet3d_main()
            P = test1.get_P()
            V = test1.V
